...
from training_loops.training_utils import rewind_model, save_initialization

class main_loop_fixed_init():
    def __init__(self, exper_config, csv_path, id_path, init_path, run_id):
    ...
    def run(self):
        full_model = FourLayerModel(self.in_dim, 16, 16, 16, self.out_dim)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        full_model.to(device)
        
        rewind_model(full_model , self.init_path)
        
        if self.width < 16:
            # perform optimal structured pruning
            model = FourLayerModel(self.in_dim, self.width, self.width, self.width, self.out_dim)
            model.to(device)
            with torch.no_grad():
                model.weight_params[0].data = full_model.weight_params[0].data[:self.width, :].contiguous()
                model.bias_params[0].data = full_model.bias_params[0][:self.width].contiguous()

                model.weight_params[1].data = full_model.weight_params[1].data[:self.width, :self.width].contiguous()
                model.bias_params[1].data = full_model.bias_params[1][:self.width].contiguous()

                model.weight_params[2].data = full_model.weight_params[2].data[:self.width, :self.width].contiguous()
                model.bias_params[2].data = full_model.bias_params[2][:self.width].contiguous()

                model.weight_params[3].data = full_model.weight_params[3].data[:, :self.width].contiguous()
                model.bias_params[3].data = full_model.bias_params[3].data
