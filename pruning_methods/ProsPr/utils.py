...
def apply_masks_with_hooks(net, keep_masks, structured, filter_fn, return_clone=True):
    ...
    for l_idx, (layer, keep_mask) in enumerate(zip(prunable_layers, keep_masks)):
        if structured:
            assert layer.weight.shape[0] == keep_mask.shape[0]
        else:
            assert layer.weight.shape == keep_mask.shape
        
        if l_idx < len(keep_mask) -1:
            if layer.bias is not None:
                b_mask = torch.sign(torch.count_nonzero(keep_masks[l_idx+1], dim=0))
                layer.bias.data.mul_(b_mask)
        
        layer.weight.data.mul_(keep_mask)
